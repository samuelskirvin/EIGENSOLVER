...
sol_omegas1e5 = [x for _,x in sorted(zip(sol_ks1,sol_omegas1))]
sol_ks1e5 = np.sort(sol_ks1)

sol_omegas1e5 = np.array(sol_omegas1e5)
sol_ks1e5 = np.array(sol_ks1e5)

sol_omegas_kink1e5 = [x for _,x in sorted(zip(sol_ks_kink1,sol_omegas_kink1))]
sol_ks_kink1e5 = np.sort(sol_ks_kink1)
...
if len(fast_sausage_branch1_omega) > 1:
  ...
  k_new = np.linspace(2.725, Kmax, num=len(FSB1_k)*10)
...
if len(fast_sausage_branch2_omega) > 1:
  ...
  k_new_2 = np.linspace(1.19, FSB2_k[-1], num=len(FSB2_k)*10)  #FSB2_k[-1]
...
if len(fast_kink_branch1_omega) > 1:
  ...
  k_kink_new = np.linspace(1.915, FSB1_kink_k[-1], num=len(FSB1_kink_k)*10)   #FSB1_kink_k[-1]
...
slow_body_sausage_branch1_omega = [0.05*(cT_i0), 0.1*(cT_i0)]
slow_body_sausage_branch1_k = [0.05, 0.1]
slow_body_sausage_branch2_omega = [0.05*(cT_i0), 0.1*(cT_i0)]
slow_body_sausage_branch2_k = [0.05, 0.1]
...
slow_body_kink_branch1_omega = [0.1*(cT_i0)]
slow_body_kink_branch1_k = [0.1]
slow_body_kink_branch2_omega = [0.1*(cT_i0)]
slow_body_kink_branch2_k = [0.1]
...
plt.savefig("Cylinder_width_1e5_coronal_curves.png")
