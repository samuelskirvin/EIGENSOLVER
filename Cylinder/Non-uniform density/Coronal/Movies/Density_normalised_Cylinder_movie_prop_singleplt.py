...
print('z[6]  =', z[6])
#exit()
THETA = np.linspace(0, 2.*np.pi, 80) #50
...
ax.contourf(x[:,:,el], y[:,:,el], PT[0,:,:,el], extend='both', cmap='bwr', alpha=0.3)  
...
def animate(i):     
   ax.clear()
   ax.set_title('$W = 3  - kink - k = 1.5$  -  z = 6  -  t = %d' %i)
   ax.plot(a,b, 'r--')
   ...
   ax.quiver(new_xx[::12,::12], new_yy[::12,::12], vx_interp[::12,::12], vy_interp[::12,::12], pivot='tail', color='black', scale_units='inches', scale=7., width=0.003)   #every 14 unless 0.9 then every 12


anim = FuncAnimation(fig, animate, interval=100, frames=len(time)-1).save('Gaussian_density_3_kink_fast_k15_propagating.mp4', writer=writer)
