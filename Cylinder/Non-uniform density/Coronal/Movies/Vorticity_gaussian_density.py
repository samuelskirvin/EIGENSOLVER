...
########################################
c_i0 = 1.
vA_e = 5.*c_i0     #5.*c_i #-coronal        #0.5*c_i -photospheric
...
test_kz = 0.1
def wA_i_01(r):                 
    return test_kz*vA_i(r)
def wC_i_01(r):              
    return test_kz*cT_i(r)

wA_i_01_np=sym.lambdify(rr,wA_i_01(rr),"numpy")   #In order to evaluate we need to switch to numpy
wC_i_01_np=sym.lambdify(rr,wC_i_01(rr),"numpy")   #In order to evaluate we need to switch to numpy
...
wA_i_05_np=sym.lambdify(rr,wA_i_05(rr),"numpy")   #In order to evaluate we need to switch to numpy
wC_i_05_np=sym.lambdify(rr,wC_i_05(rr),"numpy")   #In order to evaluate we need to switch to numpy
...
wA_i_1_np=sym.lambdify(rr,wA_i_1(rr),"numpy")   #In order to evaluate we need to switch to numpy
wC_i_1_np=sym.lambdify(rr,wC_i_1(rr),"numpy")   #In order to evaluate we need to switch to numpy
...
wA_i_2_np=sym.lambdify(rr,wA_i_2(rr),"numpy")   #In order to evaluate we need to switch to numpy
wC_i_2_np=sym.lambdify(rr,wC_i_2(rr),"numpy")   #In order to evaluate we need to switch to numpy



plt.figure()
plt.suptitle(r"$W = 1e5$", fontsize=22)
ax = plt.subplot(221)
ax2 = plt.subplot(222)
ax3 = plt.subplot(223)
ax4 = plt.subplot(224)

ax.set_title(r'$k_z = 0.1$')
ax2.set_title(r'$k_z = 0.5$')
ax3.set_title(r'$k_z = 1$')
ax4.set_title(r'$k_z = 2$')
...
ax.set_ylabel("$\omega$", fontsize=18, rotation=0, labelpad=15)
#ax2.set_ylabel("$\omega$", fontsize=18, rotation=0, labelpad=15)
ax3.set_ylabel("$\omega$", fontsize=18, rotation=0, labelpad=15)
#ax4.set_ylabel("$\omega$", fontsize=18, rotation=0, labelpad=15)

ax.plot(ix, wA_i_01_np(ix), label='$\omega_A$')
ax2.plot(ix, wA_i_05_np(ix),label='$\omega_A$')
ax3.plot(ix, wA_i_1_np(ix), label='$\omega_A$')
ax4.plot(ix, wA_i_2_np(ix), label='$\omega_A$')

ax.plot(ix, wC_i_01_np(ix), 'g', label='$\omega_C$')
ax2.plot(ix, wC_i_05_np(ix), 'g', label='$\omega_C$')
ax3.plot(ix, wC_i_1_np(ix), 'g', label='$\omega_C$')
ax4.plot(ix, wC_i_2_np(ix), 'g', label='$\omega_C$')
...
#ax.annotate( ' $v_{Ai0}$', xy=(Kmax, vA_i0), fontsize=20)
ax.legend(loc='center left')
ax2.legend(loc='center left')
ax3.legend(loc='center left')
ax4.legend(loc='center left')

plt.savefig("continuum_regions_W1e5.png")
plt.show()
exit()
...
test_k = test_arr_k[6]     #25 for roughly k=1          # 0 for k=0.1
test_w = test_arr_omega[6]    # 25 for 1.5   #20 for 3   # 6 for 0.9    # 20 for 1e5
...
time = np.linspace(0.01, 4.*np.pi, 160.)  #125   #160 5 pi
  ...
  for j in range(len(THETA)):
    for i in range(len(spatial)):
      for t in range(len(time)):
        ...
        v_z[t,i,j,k] = radial_v_z[i]*-np.sin(m*thetas[i,j,k])*np.cos(wavenum*Z[i,j,k] - w*time[t])   
        ...
        v_x[t,i,j,k] = 100.*(v_r[t,i,j,k]*np.cos(thetas[i,j,k]) - v_phi[t,i,j,k]*np.sin(thetas[i,j,k]))   # 100 for 1e5, 3, 1.5   try 400 for 0.9
        v_y[t,i,j,k] = 100.*(v_r[t,i,j,k]*np.sin(thetas[i,j,k]) + v_phi[t,i,j,k]*np.cos(thetas[i,j,k]))
