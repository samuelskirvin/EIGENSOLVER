...
slow_surf_sausage_omega = [0.01*cT_i0]
slow_surf_sausage_k = [0.01]
slow_surf_kink_omega = [0.01*cT_i0]
slow_surf_kink_k = [0.01]
...
for i in range(len(sol_ks_kink1)):   #kink mode
  ...
  elif v_phase > 0.8 and v_phase < cT_i0:
...
for i in range(len(body_sausage_omega)):   #sausage mode
  ...
  if v_phase > 0.925 and v_phase < c_i0 and body_sausage_k[i] > 2.8:
      body_sausage_branch1_omega.append(body_sausage_omega[i])
      body_sausage_branch1_k.append(body_sausage_k[i])

  if v_phase < 0.925 and v_phase > 0.906 and body_sausage_k[i] > 2.3:
...
for i in range(len(body_kink_omega)):   #kink mode
  ...
  if v_phase > 0.915 and v_phase < c_i0 and body_kink_k[i] > 2.2:
      body_kink_branch1_omega.append(body_kink_omega[i])
      body_kink_branch1_k.append(body_kink_k[i])
      
  if v_phase < 0.915 and v_phase > cT_i0 and body_kink_k[i] < 2.2:
      body_kink_branch1_omega.append(body_kink_omega[i])
      body_kink_branch1_k.append(body_kink_k[i])
...
if len(slow_surf_sausage_omega) > 1:
  ...
  SSS_k_new = np.linspace(0.4, Kmax, num=len(SSS_k)*10)
  
  SSS_coefs = poly.polyfit(SSS_k, SSS_phase, 2)
...
plt.savefig("photospheric_gaussian_density_curves_dots_15.png")
