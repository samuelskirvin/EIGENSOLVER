...
for i in range(len(sol_omegas_kink1)):
    v_ph = sol_omegas_kink1[i]/sol_ks_kink1[i]
...
test_k = test_arr_k[3]    # slow kink ka = 2 :   7 for 1e5,   4 for 3,   6 for 1.5,  3 for 1,  0 for 0.6
test_w = test_arr_omega[3]
...
#outside_v_r = -w*normalised_left_xi_solution[::-1]
#inside_v_r = -shift_freq_np(ix[::-1])*normalised_inside_xi_solution[::-1]
...
inside_xi_phi = (((g_B_np(ix[::-1])*inside_P_solution[::-1]-2.*B_i_np(ix[::-1])*T_np(ix[::-1])*(inside_xi_solution[::-1]/ix[::-1]))*(B_i_np(ix[::-1])/(rho_i0*(shift_freq_np(ix[::-1])**2 - alfven_freq_np(ix[::-1])**2)))) + ((B_iphi_np(ix[::-1])/shift_freq_np(ix[::-1])**2)*((f_B_np(ix[::-1])/rho_i0)*(c_i0**2/(c_i0**2+vA_i0**2))*((shift_freq_np(ix[::-1])**2.*inside_P_solution[::-1] - Q_np(ix[::-1])*inside_xi_solution[::-1])/(shift_freq_np(ix[::-1])**2 - cusp_freq_np(ix[::-1])**2)) - ((2.*shift_freq_np(ix[::-1])*v_iphi_np(ix[::-1])*B_iphi_np(ix[::-1]) + f_B_np(ix[::-1])*v_iphi_np(ix[::-1])**2)*(inside_xi_solution[::-1]/ix[::-1])))))/(B_i_np(ix[::-1])**2. + B_iphi_np(ix[::-1])**2.)
...
#outside_v_phi = -w*normalised_outside_xi_phi
#inside_v_phi = -(shift_freq_np(ix[::-1])*normalised_inside_xi_phi) - (dv_phi_np(ix[::-1])*ix[::-1]*normalised_inside_xi_solution[::-1])
  ...
  for j in range(len(THETA)):
    for i in range(len(spatial)):
      for t in range(len(time)):
        ...
        v_x[t,i,j,k] = 100.*(v_r[t,i,j,k]*np.cos(thetas[i,j,k]) - v_phi[t,i,j,k]*np.sin(thetas[i,j,k]))   # 100 for 1e5, 3, 0.6
...
boundary_point_x = boundary_point_r*np.cos(thetas[bound_element,:,:]) + v_x[0,bound_element,:,:]*step    
boundary_point_y = boundary_point_r*np.sin(thetas[bound_element,:,:]) + v_y[0,bound_element,:,:]*step    

boundary_point_x = boundary_point_x[0,:,:]
boundary_point_y = boundary_point_y[0,:,:]

flat_vx = v_x[0,:,:,el].flatten('C')
flat_vy = v_y[0,:,:,el].flatten('C')
...
ax.contourf(x[:,:,el], y[:,:,el], PT[0,:,:,el], extend='both', cmap='bwr', alpha=0.2)  
...
def animate(i):     
   ax.clear()
   ax.set_title('$W = 1  - slow kink - k = 2$  -  z = 6  -  t = %d' %i)
   ax.plot(a,b, 'r--')
   ...
   ax.quiver(new_xx, new_yy, vx_interp, vy_interp, pivot='tail', color='black', scale_units='inches', scale=7., width=0.003)   # scale 12 for 0.6   # 7 scale 


anim = FuncAnimation(fig, animate, interval=100, frames=len(time)-1).save('Gaussian_flow_1_kink_slow_k2.mp4', writer=writer)
