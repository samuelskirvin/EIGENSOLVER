...
#ix = np.linspace(1., 0.001, 380.)  # was this

####
#ix = np.linspace(1., 0.001, 500.)  # inside slab x values
ix = np.linspace(1., 0.15, 500.)  # inside slab x values
...
print('gauss flow freq   =', test_w) 
print('gauss flow k   =', test_k)
...
#time = np.linspace(0.01, 2.*np.pi, 80.)  

time = np.linspace(0.01, 2.*np.pi/10, 80./10)  # test
...
for t in range(len(time)):
    makeDumpVTK(x,y,z,[ xi_r[t,:,:,:], xi_phi[t,:,:,:], PT[t,:,:,:], v_r[t,:,:,:], v_phi[t,:,:,:], xi_x[t,:,:,:], xi_y[t,:,:,:], v_x[t,:,:,:], v_y[t,:,:,:], v_z[t,:,:,:], density[t,:,:,:]],varlist,'uniform_flow_slow_kink_0'+ str(t))
