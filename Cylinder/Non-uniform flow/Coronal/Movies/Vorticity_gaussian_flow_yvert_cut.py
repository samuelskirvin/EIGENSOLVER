...
#ax.set_xlim(-1.5, 1.5)
##ax.set_ylim(-1.5, 1.5)  # x-y cut
#ax.set_ylim(0., 10.)  # x-z cut
...
#
#
#ax.contourf(new_yy[el,:,:], new_zz[el,:,:], PT_interp[el,:,:], extend='both', cmap='bwr', alpha=0.3)  
##ax.contourf(flat_x[:,el,:], flat_z[:,el,:], PT[snapshot,:,el,:], extend='both', cmap='bwr', alpha=0.3)  
#ax.scatter(boundary_point_y[12,:], boundary_point_z[12,:], s=4., c='blue')  #?
#ax.plot(boundary_point_y[12,:], boundary_point_z[12,:])      #?
#ax.quiver(new_yy[el,::3,::5], new_zz[el,::3,::5], vy_interp[el,::3,::5], vz_interp[el,::3,::5], pivot='tail', color='black', scale_units='inches', scale=18., width=0.003)
#
#
##ax2.plot(a,b, 'r--')
...
##########   use for x-y cut  ####
##el_2 = 18.
##ax2.quiver(new_xx[::3,::3,el_2], new_yy[::3,::3,el_2], vort_x_comp_interp[::3,::3,el_2], vort_y_comp_interp[::3,::3,el_2], pivot='tail', color='black', scale_units='inches', scale=7., width=0.003)
...
#ax2.quiver(new_yy[el_2,::3,::2], new_zz[el_2,::3,::2], vort_y_comp_interp[el_2,::3,::2], vort_z_comp_interp[el_2,::3,::2], pivot='tail', color='black', scale_units='inches', scale=3., width=0.003)
#ax2.contourf(new_yy[el_2,:,:], new_zz[el_2,:,:], vort_x_comp_interp[el_2,:,:], extend='both', cmap='bwr', alpha=0.2)  
#
#
#
...
def animate(i):     
   ax.clear()
   ax.set_title('$W = 1  - kink - k = 2$  -  x = -0.628  -  t = %d' %i)
   #ax.plot(a,b, 'r--')
   #ax2.plot(a,b, 'r--')
   ax2.set_title('Vorticity')   
   ...
   #####   use for x-z cut
   el_2 = 210  #449.   #225 = centre
   ax2.quiver(new_yy[el_2,::3,::2], new_zz[el_2,::3,::2], vort_y_comp_interp[el_2,::3,::2], vort_z_comp_interp[el_2,::3,::2], pivot='tail', color='black', scale_units='inches', scale=1., width=0.003)
   ax2.contourf(new_yy[el_2,:,:], new_zz[el_2,:,:], vort_x_comp_interp[el_2,:,:], extend='both', cmap='bwr', alpha=0.3)  
   


anim = FuncAnimation(fig, animate, interval=100, frames=len(time)-1).save('Gaussian_flow_1_kink_slowbody_k2_vertcut_y.mp4', writer=writer)
