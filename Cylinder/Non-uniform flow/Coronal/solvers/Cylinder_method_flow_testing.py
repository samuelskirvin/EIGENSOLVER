...
#speeds = [c_i0+0.5, cT_i0-0.5, (c_i0+cT_i0)/2.]  #positive slow
#speeds = [vA_i0, vA_e, c_kink, (c_kink + vA_e)/2.]  #positive  fast

#speeds = [c_i0, vA_i0, vA_e, cT_i0, c_kink]  #positive
#speeds = [-c_i0, -vA_i0, -vA_e, -cT_i0, -c_kink]  #negative
...
##exit()  
#def alfven_freq(r):
#  return ((m*B_iphi(r)/r)+(1*B_i(r))/(sym.sqrt(rho_i(r))))
#
...
#
#def Q(r):
#  return ((-(shift_freq(r)**2 - alfven_freq(r)**2)*rho_i(r)*v_iphi(r)**2/r) + (2*shift_freq(r)**2*B_iphi(r)**2/r)+(2*shift_freq(r)*B_iphi(r)*v_iphi(r)*((m*B_iphi(r)/r)+(1*B_i(r)))/r))
#
...
#
#def T(r):
#  return ((((m*B_iphi(r)/r)+(1*B_i(r)))*B_iphi(r)) + rho_i(r)*v_iphi(r)*shift_freq(r))
#
...
def kink(wavenumber, kink_ws, kink_ks, freq):
  ...
  def locate_kink(omega, wavenum,itt_num):
      ...
      for k in range(len(omega)):
         ...
         def shift_freq(r):
           return (omega[k] - (m*v_iphi(r)/r) - wavenum*v_z(r))
         ...
         if m_e < 0:
             ...
             def objective_dPi(dPi):    # We know that Vx(0) = 0 however do not know value of dVx at boundary inside slab so use fsolve to calculate
                   ...
                   u1 = U[:,0] - (B_iphi_np(-1)**2*left_xi_solution[-1])   #- left_bound_vx    #was plus (assuming pressure is symmetric for sausage)
                   return u1[-1] 
  ...
  with stdout_redirected():
      for j in range(len(freq)):
            ...
            def shift_freq(r):
              return (freq[j] - (m*v_iphi(r)/r) - wavenumber*v_z(r))
...
wavenumber = np.linspace(0.01,4.,150.)    #101
...
if __name__ == '__main__':
    ...
    for k in wavenumber:
      for i in range(len(speeds)-1):
         ...
         test_freq = np.linspace(speeds[i]*k, speeds[i+1]*k, 70.)   #was 60
...
with open('Cylindrical_coronal_flow_00.pickle', 'wb') as f:     # EXPORT SOLUTIONS TO A PICKLE FILE
...
ax.set_ylim(cT_e-0.1, vA_i0+0.1)

box = ax.get_position()
ax.set_position([box.x0, box.y0, box.width*0.9, box.height])

ax.legend(('Method Solutions','Known Solutions'), loc = 8)   #loc='center left', bbox_to_anchor=(1, 0.))

#ax.legend(('Method Solutions','Known Solutions'), loc='center left', bbox_to_anchor=(1, 0.7))

#plt.savefig("cylinder_test_dispersion_diagram_kink.png")
                     

#Writer = animation.writers['ffmpeg']
#writer = Writer(fps=3, bitrate=4000)   #9fps if skipping 5 in time step   15 good
           
#anim = animation.ArtistAnimation(fig, image_sausage, interval=0.1, blit=True, repeat=False).save('cylinder_vr_test_sausage.mp4', writer=writer)
...
plt.ylabel(r'$\frac{\omega}{k}$', fontsize=22, rotation=0, labelpad=15)
#ax.plot(sol_ks_sausage, (sol_omegas_sausage/sol_ks_sausage), 'b.', markersize=6.)
...
#plt.savefig("cylinder_test_dispersion_diagram_sausage.png")
...
plt.savefig("cylinder_dispersion_diagram_coronal_flow_00.png")
