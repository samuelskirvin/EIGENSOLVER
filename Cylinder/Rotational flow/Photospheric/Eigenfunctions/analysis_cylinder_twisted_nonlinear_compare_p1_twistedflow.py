...
########################################
time = np.linspace(0.01, 3.*np.pi, 120.)  # test
...
fig = plt.figure(figsize=(22,8))  # try 22,8     #16,8 too narrow      22,8 good with colorbar
ax = plt.subplot(121)
ax2 = plt.subplot(122)
...
####################################################
   ...
   contour2 = ax.contourf(x[:,:,el], y[:,:,el], PT[i,:,:,el], extend='both', cmap='bwr', alpha=0.2)  
   ax2.scatter(boundary_point_x_full[:,el], boundary_point_y_full[:,el], s=4., c='blue')
   ax2.plot(boundary_point_x_full[:,el], boundary_point_y_full[:,el])     
...
anim = FuncAnimation(fig, animate, interval=100, frames=len(time)-1).save('twisted_flow_025_fast_kink_vel_full_field.mp4', writer=writer)
