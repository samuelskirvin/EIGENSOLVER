...
for i in range(len(sol_ks_v01_p125)):   #sausage mode
  v_phase = sol_omegas_v01_p125[i]/sol_ks_v01_p125[i]
      
  if v_phase > c_kink and v_phase < c_e:  
      fast_surf_sausage_omega.append(sol_omegas_v01_p125[i])
      fast_surf_sausage_k.append(sol_ks_v01_p125[i])      

  if v_phase > cT_i0 and v_phase < c_i0:  
      slow_body_sausage_omega.append(sol_omegas_v01_p125[i])
      slow_body_sausage_k.append(sol_ks_v01_p125[i])

  if v_phase > vA_e and v_phase < cT_i0:
      slow_surf_sausage_omega.append(sol_omegas_v01_p125[i])
      slow_surf_sausage_k.append(sol_ks_v01_p125[i])


amp = 0.1
alpha = 1.
test_r = 1.
for i in range(len(sol_ks_kink_v01_p125)):   #kink mode
  v_phase = sol_omegas_kink_v01_p125[i]/sol_ks_kink_v01_p125[i]
      
  if v_phase > c_kink-0.02 and v_phase < 1.33:  
      fast_surf_kink_omega.append(sol_omegas_kink_v01_p125[i])
      fast_surf_kink_k.append(sol_ks_kink_v01_p125[i])

  if v_phase < cT_i0+((amp*(1.**(alpha-1)))/(sol_ks_kink_v01_p125[i])) and sol_ks_kink_v01_p125[i] > 2. and v_phase < cT_i0 and v_phase > 0.885:  
      slow_surf_kink_omega.append(sol_omegas_kink_v01_p125[i])
      slow_surf_kink_k.append(sol_ks_kink_v01_p125[i])

  if v_phase < cT_i0+((amp*(1.**(alpha-1)))/(sol_ks_kink_v01_p125[i])) and sol_ks_kink_v01_p125[i] < 2. and v_phase < c_i0:  
      slow_surf_kink_omega.append(sol_omegas_kink_v01_p125[i])
      slow_surf_kink_k.append(sol_ks_kink_v01_p125[i])

  if v_phase > cT_i0+((amp*(1.**(alpha-1)))/(sol_ks_kink_v01_p125[i])) and v_phase < 1.2:  
      slow_body_kink_omega.append(sol_omegas_kink_v01_p125[i])
      slow_body_kink_k.append(sol_ks_kink_v01_p125[i])
...
#
#SSK_coefs = poly.polyfit(SSK_k, SSK_phase, 6)    #was 6
...
for i in range(len(slow_body_kink_omega)):   #kink mode
  ...
  if v_phase > 0.9455 and v_phase < 1.2 and slow_body_kink_k[i] > 2.2:
...
KB_k_new = np.linspace(KB_k[0], Kmax, num=len(KB_k)*1)   
...
if len(body_kink_branch2_omega) > 1:
   ...
   KB2_k_new = np.linspace(2.1, Kmax, num=len(KB2_k)*10)
...
plt.savefig("v01_p125_photospheric_curves.png")
plt.show()
exit()
