...
slow_body_kink_omega = []
slow_body_kink_k = []
slow_surf2_kink_omega = []
slow_surf2_kink_k = []


for i in range(len(sol_ks_v01_p1)):   #sausage mode
  v_phase = sol_omegas_v01_p1[i]/sol_ks_v01_p1[i]
      
  if v_phase > 1.32 and v_phase < c_e and sol_ks_v01_p1[i] > 1.5:  
      fast_surf_sausage_omega.append(sol_omegas_v01_p1[i])
      fast_surf_sausage_k.append(sol_ks_v01_p1[i])      

  if v_phase > 1.45 and v_phase < c_e and sol_ks_v01_p1[i] < 1.5:  
      fast_surf_sausage_omega.append(sol_omegas_v01_p1[i])
      fast_surf_sausage_k.append(sol_ks_v01_p1[i])      

  if v_phase > cT_i0 and v_phase < c_i0:  
      slow_body_sausage_omega.append(sol_omegas_v01_p1[i])
      slow_body_sausage_k.append(sol_ks_v01_p1[i])

  if v_phase > vA_e and v_phase < cT_i0:
      slow_surf_sausage_omega.append(sol_omegas_v01_p1[i])
      slow_surf_sausage_k.append(sol_ks_v01_p1[i])


amp = 0.1
alpha = 1.
test_r = 1.
for i in range(len(sol_ks_kink_v01_p1)):   #kink mode
  v_phase = sol_omegas_kink_v01_p1[i]/sol_ks_kink_v01_p1[i]
      
  if v_phase > c_kink and v_phase < c_e and v_phase > cT_i0+((amp*(1.**(alpha-1)))/(sol_ks_kink_v01_p1[i])):  
      fast_surf_kink_omega.append(sol_omegas_kink_v01_p1[i])
      fast_surf_kink_k.append(sol_ks_kink_v01_p1[i])

  if v_phase < cT_i0+((amp*(1.**(alpha-1)))/(sol_ks_kink_v01_p1[i])) and v_phase > cT_i0 and v_phase < c_kink: # and sol_ks_kink_v01_p1[i] < 2.:  
      slow_surf_kink_omega.append(sol_omegas_kink_v01_p1[i])
      slow_surf_kink_k.append(sol_ks_kink_v01_p1[i])

#  if v_phase < cT_i0+((amp*(1.**(alpha-1)))/(sol_ks_kink_v01_p1[i])) and sol_ks_kink_v01_p1[i] > 2.8 and v_phase < 0.924:  
#      slow_surf_kink_omega.append(sol_omegas_kink_v01_p1[i])
#      slow_surf_kink_k.append(sol_ks_kink_v01_p1[i])

  if v_phase > cT_i0+((amp*(1.**(alpha-1)))/(sol_ks_kink_v01_p1[i])) and v_phase < c_i0:  
      slow_body_kink_omega.append(sol_omegas_kink_v01_p1[i])
      slow_body_kink_k.append(sol_ks_kink_v01_p1[i])
      
####
  if sol_ks_kink_v01_p1[i] < 0.17 and v_phase > c_kink:  
      slow_surf2_kink_omega.append(sol_omegas_kink_v01_p1[i])
      slow_surf2_kink_k.append(sol_ks_kink_v01_p1[i])

  if sol_ks_kink_v01_p1[i] < 0.25 and v_phase > c_kink and v_phase < 1.32:  
      slow_surf2_kink_omega.append(sol_omegas_kink_v01_p1[i])
      slow_surf2_kink_k.append(sol_ks_kink_v01_p1[i])
...
FSK_k_new = np.linspace(0.165, 3.96, num=len(FSK_k)*10)

FSK_coefs = poly.polyfit(FSK_k, FSK_phase, 35)    #was 6
...
if len(slow_surf_sausage_omega) > 1:
  ...
  SSS_coefs = poly.polyfit(SSS_k, SSS_phase, 10)
...
SSK_k_new = np.linspace(0.25, Kmax, num=len(SSK_k)*10)

SSK_coefs = poly.polyfit(SSK_k, SSK_phase, 16)    #was 6
SSK_ffit = poly.polyval(SSK_k_new, SSK_coefs)

##############

slow_surf2_kink_omega = np.array(slow_surf2_kink_omega)
slow_surf2_kink_k = np.array(slow_surf2_kink_k)

SS2K_phase = slow_surf2_kink_omega/slow_surf2_kink_k  #combined fast surface and body kink
SS2K_k = slow_surf2_kink_k
SS2K_k_new = np.linspace(SS2K_k[0], 0.25, num=len(SS2K_k)*10)

SS2K_coefs = poly.polyfit(SS2K_k, SS2K_phase, 8)    #was 6
SS2K_ffit = poly.polyval(SS2K_k_new, SS2K_coefs)
...
for i in range(len(slow_body_sausage_omega)):   #sausage mode
  ...
  if v_phase > 0.92 and v_phase < c_i0:
      slow_body_sausage_branch1_omega.append(slow_body_sausage_omega[i])
      slow_body_sausage_branch1_k.append(slow_body_sausage_k[i])

  if v_phase < 0.92 and v_phase > 0.904 and slow_body_sausage_k[i] < 2.:
      slow_body_sausage_branch1_omega.append(slow_body_sausage_omega[i])
      slow_body_sausage_branch1_k.append(slow_body_sausage_k[i])

  if v_phase < 0.925 and v_phase > 0.907 and slow_body_sausage_k[i] > 2.75:
...
for i in range(len(slow_body_kink_omega)):   #kink mode
  ...
  if v_phase > 0.95 and v_phase < c_i0 and slow_body_kink_k[i] > 1.6:
      body_kink_branch1_omega.append(slow_body_kink_omega[i])
      body_kink_branch1_k.append(slow_body_kink_k[i])
      
  if v_phase > cT_i0+((amp*(1.**(alpha-1)))/(slow_body_kink_k[i])) and v_phase < c_i0 and slow_body_kink_k[i] < 1.6:
      body_kink_branch1_omega.append(slow_body_kink_omega[i])
      body_kink_branch1_k.append(slow_body_kink_k[i])

  if v_phase > cT_i0+((amp*(1.**(alpha-1)))/(slow_body_kink_k[i])) and v_phase < 0.95 and v_phase > 0.933:
      body_kink_branch2_omega.append(slow_body_kink_omega[i])
      body_kink_branch2_k.append(slow_body_kink_k[i])

#  if v_phase < 0.906 and v_phase > 0.903 and slow_body_kink_k[i] < 2.7 and slow_body_kink_k[i] > 1.01:
#      body_kink_branch2_omega.append(slow_body_kink_omega[i])
#      body_kink_branch2_k.append(slow_body_kink_k[i])
...
KB_k_new = np.linspace(0.98, Kmax, num=len(KB_k)*1)   

KB_coefs = poly.polyfit(KB_k, KB_phase, 5)   
...
if len(body_kink_branch2_omega) > 1:
   ...
   KB2_k_new = np.linspace(1.32, KB2_k[-1], num=len(KB2_k)*10)
...
#ax.plot(sol_ks_v01_p1, (sol_omegas_v01_p1/sol_ks_v01_p1), 'r.', markersize=4.)
...
ax.plot(SS2K_k_new, SS2K_ffit, color='b')    
ax.plot(KB_k_new, KB_ffit, color='b')    
ax.plot(KB2_k_new, KB2_ffit, color='b')

###########

#ax.plot(body_kink_branch2_k, (body_kink_branch2_omega/body_kink_branch2_k), 'kx', markersize=10.)

ax.fill_between(test_k, cT_i0+((amp*(0.002**(alpha-1)))/(test_k)), cT_i0+((amp*(test_r**(alpha-1)))/(test_k)), color='green', alpha=0.4)  

plt.savefig("v01_p1_photospheric_curves_KINKONLY.png")
plt.show()
exit()
...
#    if (sol_omegas_kink_v01_p1[i]/sol_ks_kink_v01_p1[i]) < (cT_i0+(0.1/sol_ks_kink_v01_p1[i])):
#       Omega_slow_kink_w.append(sol_omegas_kink_v01_p1[i])
#       Omega_slow_kink_k.append(sol_ks_kink_v01_p1[i])
#       
#    if (sol_omegas_kink_v01_p1[i]/sol_ks_kink_v01_p1[i]) > c_i0 and (sol_omegas_kink_v01_p1[i]/sol_ks_kink_v01_p1[i]) > (cT_i0+(0.1/sol_ks_kink_v01_p1[i])):
#       Omega_fast_kink_w.append(sol_omegas_kink_v01_p1[i])
#       Omega_fast_kink_k.append(sol_ks_kink_v01_p1[i])
#
#    if (sol_omegas_kink_v01_p1[i]/sol_ks_kink_v01_p1[i]) < c_i0 and (sol_omegas_kink_v01_p1[i]/sol_ks_kink_v01_p1[i]) > (cT_i0+(0.1/sol_ks_kink_v01_p1[i])):
#       Omega_body_kink_w.append(sol_omegas_kink_v01_p1[i])
#       Omega_body_kink_k.append(sol_ks_kink_v01_p1[i])
#
#
...
############################
#
#wavenum = Omega_fast_kink_k[::-1]
#frequency = Omega_fast_kink_w[::-1]
#
######################################################
...
#alpha = 1.
#test_r = 1.
#ax.fill_between(test_k, cT_i0+((0.1*(0.002**(alpha-1)))/(test_k)), cT_i0+((0.1*(test_r**(alpha-1)))/(test_k)), color='red', alpha=0.2)    # fill between uniform case and boundary values
#
...
#
#box = ax.get_position()
#ax.set_position([box.x0, box.y0-0.18, box.width*1., box.height*2.])
#
#box2 = ax2.get_position()
#ax2.set_position([box2.x0, box2.y0-0.15, box2.width*1., box2.height*0.65])
#
#box3 = ax3.get_position()
#ax3.set_position([box3.x0, box3.y0-0.06, box3.width*1., box3.height*0.65])
#
...
#   w = frequency[i]
#
#   disp_dot, = ax.plot(wavenum[i], frequency[i]/wavenum[i], 'r.', markersize=16.)
#
#   def f_B(r): 
...
#   xi_plot, = ax3.plot(spatial_v015_p125, normalised_radial_displacement_v015_p125, 'b') 
#   
#   ######  v_r   #######
##   
...
#   ######################
#
#   image.append([disp_dot, pressure_plot, xi_plot]) #, v_z_plot, v_phi_plot, v_phi_text, v_phi_text2, v_z_text, v_z_text2])
#       
#
#Writer = animation.writers['ffmpeg']
#writer = Writer(fps=3, bitrate=4000)   #9fps if skipping 5 in time step   15 good
#           
#anim = animation.ArtistAnimation(fig, image, interval=0.1, blit=True, repeat=False).save('v01_p1_fastsurf_kink_v_components_thesis.mp4', writer=writer)
#
#
#plt.show()
#exit()
...
for i in range(len(sol_ks_kink_v01_p1)):
    ...
    if sol_ks_kink_v01_p1[i] > 0.56 and sol_ks_kink_v01_p1[i] < 0.62:         
...
wavenum = sol_ksv005_p1_singleplot_slow_surf_kink[0]
frequency = sol_omegasv005_p1_singleplot_slow_surf_kink[0]
...
wavenum = sol_ksv015_p1_singleplot_slow_surf_kink[0]
frequency = sol_omegasv015_p1_singleplot_slow_surf_kink[0]
...
ax.set_title(r' slow surface kink mode,  $k = 0.6$  ,  $v_{\varphi} \propto r^{1}$')
...
plt.savefig('Nonlinear_vrot_p1_slowsurfkink_eigenfunctions.png')
