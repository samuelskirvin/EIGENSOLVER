...
sol_ksv01_p08_singleplot_body_kink = []
sol_omegasv01_p08_singleplot_body_kink = []
...
for i in range(len(sol_ks_kink_v01_p08)):
    ...
    if sol_ks_kink_v01_p08[i] > 1.65 and sol_ks_kink_v01_p08[i] < 1.7:         
       if sol_omegas_kink_v01_p08[i]/sol_ks_kink_v01_p08[i] > cT_i0 and sol_omegas_kink_v01_p08[i]/sol_ks_kink_v01_p08[i] < 0.96: 
    ...
    if sol_ks_kink_v01_p08[i] > 1.65 and sol_ks_kink_v01_p08[i] < 1.7:         
       if sol_omegas_kink_v01_p08[i]/sol_ks_kink_v01_p08[i] > 0.96 and sol_omegas_kink_v01_p08[i]/sol_ks_kink_v01_p08[i] < 0.97: 
         sol_ksv01_p08_singleplot_body_kink.append(sol_ks_kink_v01_p08[i])
         sol_omegasv01_p08_singleplot_body_kink.append(sol_omegas_kink_v01_p08[i])
...
sol_ksv01_p08_singleplot_body_kink = np.array(sol_ksv01_p08_singleplot_body_kink)
sol_omegasv01_p08_singleplot_body_kink = np.array(sol_omegasv01_p08_singleplot_body_kink)
...
for i in range(len(sol_ks_kink_v025_p08)):
    ...
    if sol_ks_kink_v025_p08[i] > 0.56 and sol_ks_kink_v025_p08[i] < 0.62:         
...
ax.plot(sol_ksv01_p08_singleplot_slow_surf_kink, (sol_omegasv01_p08_singleplot_slow_surf_kink/sol_ksv01_p08_singleplot_slow_surf_kink), 'k.', markersize=15.)
...
wavenum = sol_ksv01_p08_singleplot_slow_surf_kink[0]
frequency = sol_omegasv01_p08_singleplot_slow_surf_kink[0]
...
ax.plot(spatial_v01_p08, radial_PT_v01_p08, 'k')
...
ax2.plot(spatial_v01_p08, radial_displacement_v01_p08, 'k')
...
ax3.plot(spatial_v01_p08, radial_xi_phi_v01_p08, 'k')
  ...
  for j in range(len(THETA)):
    for i in range(len(spatial)):
      for t in range(len(time)):
        xi_r[t,i,j,k] = radial_displacement_v01_p08[i]*np.cos(m*thetas[i,j,k])*np.cos(wavenum*Z[i,j,k] - w*time[t])  
        xi_phi[t,i,j,k] = radial_xi_phi_v01_p08[i]*-np.sin(m*thetas[i,j,k])*np.cos(wavenum*Z[i,j,k] - w*time[t])        
        #xi_z[t,i,j,k] = radial_xi_z[i]*np.cos(m*thetas[i,j,k])*np.cos(wavenum*Z[i,j,k])*np.sin(w*time[t])
        PT[t,i,j,k] = radial_PT_v01_p08[i]*np.cos(m*thetas[i,j,k])*np.cos(wavenum*Z[i,j,k] - w*time[t]) 
        v_r[t,i,j,k] = 3e5*(radial_vr_v01_p08[i]*np.cos(m*thetas[i,j,k])*np.cos(wavenum*Z[i,j,k] - w*time[t]))
        v_phi[t,i,j,k] = 3e5*(radial_v_phi_v01_p08[i]*-np.sin(m*thetas[i,j,k])*np.cos(wavenum*Z[i,j,k] - w*time[t]))
        v_z[t,i,j,k] = 3e5*(radial_v_z_v01_p08[i]*-np.sin(m*thetas[i,j,k])*np.cos(wavenum*Z[i,j,k] - w*time[t]))
...
def animate(i):     
   ...
   ax.set_title(' $A = 0.1  -  p = 0.8  - kink - k = 1.65$ - t = %d' %i)
   ax.plot(a,b, 'r--')
   ax2.plot(a,b, 'r--')
...
anim = FuncAnimation(fig, animate, interval=100, frames=len(time)-1).save('twisted_flow_v01_p08_slow_surf_kink_vel_full_field.mp4', writer=writer)
