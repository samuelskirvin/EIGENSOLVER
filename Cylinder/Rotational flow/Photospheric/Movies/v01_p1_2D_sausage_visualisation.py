...
plt.figure()
ax = plt.subplot(111)
ax.set_title(r'$\propto 0.25*r^{1}$')
...
ax.plot(sol_ks_v025_p1, (sol_omegas_v025_p1/sol_ks_v025_p1), 'r.', markersize=4.)
...
for i in range(len(sol_ks_v005_p1)):
    ...
    if sol_ks_v005_p1[i] > 0.55 and sol_ks_v005_p1[i] < 0.62:         
...
for i in range(len(sol_ks_v025_p1)):
    ...
    if sol_ks_v025_p1[i] > 0.55 and sol_ks_v025_p1[i] < 0.62:         
...
ax.plot(sol_ksv01_p1_singleplot_slow_surf_sausage, (sol_omegasv01_p1_singleplot_slow_surf_sausage/sol_ksv01_p1_singleplot_slow_surf_sausage), 'b.', markersize=15.)
...
wavenum = sol_ksv01_p1_singleplot_slow_surf_sausage[0]
frequency = sol_omegasv01_p1_singleplot_slow_surf_sausage[0]
...
lx = np.linspace(2.*2.*np.pi/wavenum, 1., 2500.)  # Number of wavelengths/2*pi accomodated in the domain   #1.75 before
...
spatial_v01_p1 = np.concatenate((ix[::-1], lx[::-1]), axis=None)     

radial_displacement_v01_p1 = np.concatenate((inside_xi_solution[::-1], left_xi_solution[::-1]), axis=None)  
radial_PT_v01_p1 = np.concatenate((inside_P_solution[::-1], left_P_solution[::-1]), axis=None)  

normalised_radial_displacement_v01_p1 = np.concatenate((normalised_inside_xi_solution[::-1], normalised_left_xi_solution[::-1]), axis=None)  
normalised_radial_PT_v01_p1 = np.concatenate((normalised_inside_P_solution[::-1], normalised_left_P_solution[::-1]), axis=None)  
...
radial_vr_v01_p1 = np.concatenate((inside_v_r, outside_v_r), axis=None)    
...
normalised_radial_vr_v01_p1 = np.concatenate((normalised_inside_v_r, normalised_outside_v_r), axis=None)    
...
radial_xi_z_v01_p1 = np.concatenate((inside_xi_z, outside_xi_z), axis=None) 
...
radial_xi_phi_v01_p1 = np.concatenate((inside_xi_phi, outside_xi_phi), axis=None)    
...
normalised_radial_xi_phi_v01_p1 = np.concatenate((normalised_inside_xi_phi, normalised_outside_xi_phi), axis=None)    
...
radial_v_phi_v01_p1 = np.concatenate((inside_v_phi, outside_v_phi), axis=None)    
...
normalised_radial_v_phi_v01_p1 = np.concatenate((normalised_inside_v_phi, normalised_outside_v_phi), axis=None)    
...
radial_v_z_v01_p1 = np.concatenate((inside_v_z, outside_v_z), axis=None)    
...
ax.plot(spatial_v01_p1, radial_PT_v01_p1, 'k')
...
ax2.plot(spatial_v01_p1, radial_displacement_v01_p1, 'k')
...
ax3.plot(spatial_v01_p1, radial_xi_phi_v01_p1, 'k')
...
spatial=spatial_v01_p1
########################################
time = np.linspace(0.01, 2.5*np.pi, 100.)  # 2.5pi and 100
  ...
  for j in range(len(THETA)):
    for i in range(len(spatial)):
      for t in range(len(time)):
        xi_r[t,i,j,k] = radial_displacement_v01_p1[i]*np.cos(m*thetas[i,j,k])*np.cos(wavenum*Z[i,j,k] - w*time[t])  
        xi_phi[t,i,j,k] = radial_xi_phi_v01_p1[i]*-np.sin(m*thetas[i,j,k])*np.cos(wavenum*Z[i,j,k] - w*time[t])        
        #xi_z[t,i,j,k] = radial_xi_z[i]*np.cos(m*thetas[i,j,k])*np.cos(wavenum*Z[i,j,k])*np.sin(w*time[t])
        PT[t,i,j,k] = radial_PT_v01_p1[i]*np.cos(m*thetas[i,j,k])*np.cos(wavenum*Z[i,j,k] - w*time[t]) 
        v_r[t,i,j,k] = 1e5*(radial_vr_v01_p1[i]*np.cos(m*thetas[i,j,k])*np.cos(wavenum*Z[i,j,k] - w*time[t]))
        v_phi[t,i,j,k] = 1e5*(radial_v_phi_v01_p1[i]*-np.sin(m*thetas[i,j,k])*np.cos(wavenum*Z[i,j,k] - w*time[t]))
        v_z[t,i,j,k] = 1e5*(radial_v_z_v01_p1[i]*-np.sin(m*thetas[i,j,k])*np.cos(wavenum*Z[i,j,k] - w*time[t]))

        bkg_vel_x_01[i,j,k] = 2.*bkg_v_iphi_01[i]*-np.sin(thetas[i,j,k])  #v_iphi_005_np(ix)[i]*-np.sin(thetas[i,j,k])
        bkg_vel_y_01[i,j,k] = 2.*bkg_v_iphi_01[i]*np.cos(thetas[i,j,k])  #v_iphi_005_np(ix)[i]*np.cos(thetas[i,j,k]) 
...
def animate(i):     
   ...
   ax.set_title(' Velocity $A = 0.1  - sausage - k = 1.6$ - t = %d' %i)
   ax.plot(a,b, 'r--')
   ax2.plot(a,b, 'r--')
...
#####
   ...
   ax.scatter(boundary_point_x[:,el], boundary_point_y[:,el], s=4., c='blue')
   ax.plot(boundary_point_x[:,el], boundary_point_y[:,el])     
   ax.quiver(new_xx[::20,::20], new_yy[::20,::20], vx_interp[::20,::20], vy_interp[::20,::20], pivot='tail', color='black', scale_units='inches', scale=2., width=0.003)   # 7 scale 
...
####################################################
   ...
   ax2.scatter(boundary_point_x_full[:,el], boundary_point_y_full[:,el], s=4., c='blue')
   ax2.plot(boundary_point_x_full[:,el], boundary_point_y_full[:,el])     
   ax2.quiver(new_xx[::20,::20], new_yy[::20,::20], vx_interp_full[::20,::20], vy_interp_full[::20,::20], pivot='tail', color='black', scale_units='inches', scale=2., width=0.003)
...
anim = FuncAnimation(fig, animate, interval=100, frames=len(time)-1).save('twisted_flow_v01_p1_slow_sausage_vel_full_field.mp4', writer=writer)
