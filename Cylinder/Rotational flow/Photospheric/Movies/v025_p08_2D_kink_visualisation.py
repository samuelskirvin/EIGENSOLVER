...
for i in range(len(sol_ks_v001_p08)):
    ...
    if sol_ks_v001_p08[i] > 1.29 and sol_ks_v001_p08[i] < 1.45:         
       if sol_omegas_v001_p08[i]/sol_ks_v001_p08[i] > 0.905 and sol_omegas_v001_p08[i]/sol_ks_v001_p08[i] < 0.92: 
...
for i in range(len(sol_ks_kink_v001_p08)):
    if sol_ks_kink_v001_p08[i] > 0.68 and sol_ks_kink_v001_p08[i] < 0.7:        
    ...
    if sol_ks_kink_v001_p08[i] > 0.56 and sol_ks_kink_v001_p08[i] < 0.62:         
...
for i in range(len(sol_ks_kink_v005_p08)):
    if sol_ks_kink_v005_p08[i] > 0.68 and sol_ks_kink_v005_p08[i] < 0.7:        
    ...
    if sol_ks_kink_v005_p08[i] > 0.56 and sol_ks_kink_v005_p08[i] < 0.62:         
...
for i in range(len(sol_ks_kink_v01_p08)):
    if sol_ks_kink_v01_p08[i] > 0.68 and sol_ks_kink_v01_p08[i] < 0.7:        
    ...
    if sol_ks_kink_v01_p08[i] > 0.56 and sol_ks_kink_v01_p08[i] < 0.62:         
...
for i in range(len(sol_ks_kink_v015_p08)):
    if sol_ks_kink_v015_p08[i] > 0.68 and sol_ks_kink_v015_p08[i] < 0.7:        
    ...
    if sol_ks_kink_v015_p08[i] > 0.56 and sol_ks_kink_v015_p08[i] < 0.62:         
...
for i in range(len(sol_ks_kink_v025_p08)):
    if sol_ks_kink_v025_p08[i] > 0.68 and sol_ks_kink_v025_p08[i] < 0.7:        
    ...
    if sol_ks_kink_v025_p08[i] > 0.65 and sol_ks_kink_v025_p08[i] < 0.7:         
       if sol_omegas_kink_v025_p08[i]/sol_ks_kink_v025_p08[i] > c_i0 and sol_omegas_kink_v025_p08[i]/sol_ks_kink_v025_p08[i] < 1.3: 
...
def v_iphi_025(r):  
    return v_twist_025*(r**0.8)   #0.

v_iphi_025_np=sym.lambdify(rr,v_iphi_025(rr),"numpy")   #In order to evaluate we need to switch to numpy

outisde_bkg_v_phi_025 = np.zeros(len(lx))

bkg_v_iphi_025 = np.concatenate((v_iphi_025_np(ix[::-1]), outisde_bkg_v_phi_025), axis=None)    
...
########################################
time = np.linspace(0.01, 2.5*np.pi, 100.)  # test
...
bkg_vel_x_025 = np.zeros(((len(spatial), len(THETA), len(z))))
bkg_vel_y_025 = np.zeros(((len(spatial), len(THETA), len(z))))
  ...
  for j in range(len(THETA)):
    for i in range(len(spatial)):
      for t in range(len(time)):
        ...
        v_r[t,i,j,k] = 5e4*(radial_vr_v025_p1[i]*np.cos(m*thetas[i,j,k])*np.cos(wavenum*Z[i,j,k] - w*time[t]))
        v_phi[t,i,j,k] = 5e4*(radial_v_phi_v025_p1[i]*-np.sin(m*thetas[i,j,k])*np.cos(wavenum*Z[i,j,k] - w*time[t]))
        v_z[t,i,j,k] = 5e4*(radial_v_z_v025_p1[i]*-np.sin(m*thetas[i,j,k])*np.cos(wavenum*Z[i,j,k] - w*time[t]))

        bkg_vel_x_025[i,j,k] = 10.*bkg_v_iphi_025[i]*-np.sin(thetas[i,j,k])  #v_iphi_005_np(ix)[i]*-np.sin(thetas[i,j,k])
        bkg_vel_y_025[i,j,k] = 10.*bkg_v_iphi_025[i]*np.cos(thetas[i,j,k])  #v_iphi_005_np(ix)[i]*np.cos(thetas[i,j,k]) 
...
def animate(i):     
   ...
   ax.set_title(' Velocity $A = 0.25  - kink - k = 0.55$ - t = %d' %i)
   ax.plot(a,b, 'r--')
   ax2.plot(a,b, 'r--')
...
#####
   ...
   ax.scatter(boundary_point_x[:,el], boundary_point_y[:,el], s=4., c='blue')
   ax.plot(boundary_point_x[:,el], boundary_point_y[:,el])     
   ax.quiver(new_xx[::10,::10], new_yy[::10,::10], vx_interp[::10,::10], vy_interp[::10,::10], pivot='tail', color='black', scale_units='inches', scale=5., width=0.003)   # 7 scale 
...
####################################################
   ...
   boundary_point_x_full = boundary_point_r*np.cos(thetas[bound_element,:,:]) + (v_x[i,bound_element,:,:] + bkg_vel_x_025[bound_element,:,:])*step    
   boundary_point_y_full = boundary_point_r*np.sin(thetas[bound_element,:,:]) + (v_y[i,bound_element,:,:] + bkg_vel_y_025[bound_element,:,:])*step    
   ...
   flat_vx_full = (v_x[i,:,:,el] + bkg_vel_x_025[:,:,el]).flatten('C')
   flat_vy_full = (v_y[i,:,:,el] + bkg_vel_y_025[:,:,el]).flatten('C')
   ...
   ax2.scatter(boundary_point_x_full[:,el], boundary_point_y_full[:,el], s=4., c='blue')
   ax2.plot(boundary_point_x_full[:,el], boundary_point_y_full[:,el])     
   ax2.quiver(new_xx[::10,::10], new_yy[::10,::10], vx_interp_full[::10,::10], vy_interp_full[::10,::10], pivot='tail', color='black', scale_units='inches', scale=5., width=0.003)
...
anim = FuncAnimation(fig, animate, interval=100, frames=len(time)-1).save('twisted_flow_v025_p08_fast_kink_vel_full_field.mp4', writer=writer)
