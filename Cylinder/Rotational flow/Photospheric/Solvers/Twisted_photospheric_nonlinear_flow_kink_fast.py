...
speeds = [c_kink, 1.35, 1.4]  #added -c_e and cT_i0
...
#wavenumber = np.linspace(0.25,4.,100)    #  0.15 works for > 1
wavenumber = np.linspace(0.25,0.37,20)    #  0.15 works for > 1
...
if __name__ == '__main__':
    ...
    for k in wavenumber:
      for i in range(len(speeds)-1):
         ...
         test_freq = np.linspace(speeds[i]*k, speeds[i+1]*k, 50.)   #was 60
...
with open('Cylindrical_photospheric_vtwist025_power08_fund_kink2.pickle', 'wb') as f:     # EXPORT SOLUTIONS TO A PICKLE FILE
...
plt.savefig("cylinder_photospheric_vtwist025_power08_fund_kink2.png")
