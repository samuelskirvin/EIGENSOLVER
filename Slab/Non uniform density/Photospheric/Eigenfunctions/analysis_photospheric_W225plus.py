...
index_to_remove = []
for i in range(len(slow_surf_kink_omega)-1):
  ph_diff = abs((slow_surf_kink_omega[i+1]/slow_surf_kink_k[i+1]) - (slow_surf_kink_omega[i]/slow_surf_kink_k[i]))
  
  if ph_diff > 0.015:  
      index_to_remove.append(i)
      
slow_surf_kink_omega = np.delete(slow_surf_kink_omega, index_to_remove)
slow_surf_kink_k = np.delete(slow_surf_kink_k, index_to_remove)
...
if len(body_sausage_branch2_omega) > 1:
  ...
  SB2_coefs = poly.polyfit(SB2_k, SB2_phase, 6)   #was 6    # 1 good for W > 3
...
KB_coefs = poly.polyfit(KB_k, KB_phase, 6)     # use 6    # use 1 for W <=1.75
...
if len(slow_surf_kink_omega) > 1:
  ...
  SSK_coefs = poly.polyfit(SSK_k, SSK_phase, 8)   #was 6     #use 2 for W=2.5
...
ax.plot(SSK_k_new, SSK_ffit, color='b')    #comment out for W < 2.1
...
plt.savefig("photospheric_curves_dots_shaded_3mod.png")
