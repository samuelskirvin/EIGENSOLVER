...
#####################################################
#speeds = [-c_e, 0., c_i, c_e, vA_i, cT_i(), -vA_i]

speeds = [-0.5, 0.5, 0., 1.]    #modified for extra res in imaginary region 
...
#      
#      plt.plot(wavenumber[i], (freq[j] + 1j*freq[m])/wavenumber[i], 'b.')
#
...
#  for j in range(len(freq)):
#      
#      plt.plot(wavenumber[i], (freq[j] + 1j*freq[m]), 'b.')
# 
...
#####    DEFINE FUNCTIONS TO LOCATE SOLUTIONS    #####################
  ...
  def locate_sausage(omega, wavenum,itt_num):
      ...
      for k in range(len(omega.real)):
        for n in range(len(omega.imag)):
          ...
          m_e = ((((wavenum**2*vA_e**2)-((omega[k] + 1j*omega[n])-(wavenum*mach_e))**2)*((wavenum**2*c_e**2)-((omega[k] + 1j*omega[n])-(wavenum*mach_e))**2))/((vA_e**2+c_e**2)*((wavenum**2*cT_e()**2)-((omega[k] + 1j*omega[n])-(wavenum*mach_e))**2)))
       
          p_e_const = rho_e*(vA_e**2+c_e**2)*((wavenum**2*cT_e()**2)-((omega[k] + 1j*omega[n])-(wavenum*mach_e))**2)/(((omega[k] + 1j*omega[n])-(wavenum*mach_e))*((wavenum**2*c_e**2)-((omega[k] + 1j*omega[n])-(wavenum*mach_e))**2))       
 
 
 #######################################################################################################           
          def m0(x):    
            return ((((wavenum**2*c_i**2)-((omega[k] + 1j*omega[n])-(wavenum*U_i(x)))**2)*((wavenum**2*vA_i**2)-((omega[k] + 1j*omega[n])-(wavenum*U_i(x)))**2))/((c_i**2+vA_i**2)*((wavenum**2*cT_i()**2)-((omega[k] + 1j*omega[n])-(wavenum*U_i(x)))**2)))  
  
          m0_np=sym.lambdify(xx,m0(xx),"numpy")           
     ##############################################
                
          #def D(x):    
           # return (2.*wavenum*dU_i(x)*((((omega[k] + 1j*omega[n])-(wavenum*U_i(x)))**2-(wavenum**2*cT_i()**2))+((wavenum**4*cT_i()**2*c_i**2)/((c_i**2+vA_i**2)*(((omega[k] + 1j*omega[n])-(wavenum*U_i(x)))**2-(wavenum**2*cT_i()**2)))))/(((omega[k] + 1j*omega[n])-wavenum*U_i(x))*(((omega[k] + 1j*omega[n])-(wavenum*U_i(x)))**2-(wavenum**2*c_i**2))))           
 
          def D(x):    
            return (2.*wavenum*dU_i(x)*((((omega[k] + 1j*omega[n])-wavenum*U_i(x))**2/((((omega[k] + 1j*omega[n])-wavenum*U_i(x))**2)-wavenum**2*c_i**2)) - ((wavenum**2*cT_i()**2)/((((omega[k] + 1j*omega[n])-wavenum*U_i(x))**2)-wavenum**2*cT_i()**2)))/((omega[k] + 1j*omega[n])-wavenum*U_i(x)))  
          
          D_np=sym.lambdify(xx,D(xx),"numpy")
     ##############################################
     
          def coeff(x):
              return ((wavenum*ddU_i(x)/((omega[k] + 1j*omega[n])-wavenum*U_i(x)))+((wavenum*dU_i(x)*D(x))/((omega[k] + 1j*omega[n])-wavenum*U_i(x))) - m0(x))
 
          coeff_np=sym.lambdify(xx,coeff(xx),"numpy")
     ##############################################
     
          def P_Ti(x):    
            return (rho_i*(vA_i**2+c_i**2)*((wavenum**2*cT_i()**2)-((omega[k] + 1j*omega[n])-(wavenum*U_i(x)))**2)/(((omega[k] + 1j*omega[n])-(wavenum*U_i(x)))*((wavenum**2*c_i**2)-((omega[k] + 1j*omega[n])-(wavenum*U_i(x)))**2))) 
           
          PT_i_np=sym.lambdify(xx,P_Ti(xx),"numpy")
          p_i_const = PT_i_np(ix)
 
          def add_P_Ti(x):
            return (-(wavenum*dU_i(x))/((omega[k] + 1j*omega[n])-wavenum*U_i(x)))
 ...
 ########################################################################################################
          ...
          if m_e.real < 0:
              ...
              left_solution = Ls[:,0].real*((omega[k])-wavenum*mach_i[0])/((omega[k])-wavenum*mach_e)   
              left_solution_imag = Ls[:,0].imag*((omega[n])-wavenum*mach_i[0])/((omega[n])-wavenum*mach_e) 
              ...
              P_diff_loop_check_imag.append((left_P_solution_imag[-1]-inside_P_solution_imag[0]))
              loop_sign_check_imag.append(P_diff_loop_check_imag[-1]*P_diff_loop_check_imag[-2])
              
              
              if (abs(left_P_solution[-1] - inside_P_solution[0])*100/max(abs(left_P_solution[-1]), abs(inside_P_solution[0]))) < p_tol: # and (abs(left_P_solution_imag[-1] - inside_P_solution_imag[0])*100/max(abs(left_P_solution_imag[-1]), abs(inside_P_solution_imag[0]))) < p_tol:
                     sol_omegas1.append(omega[k])
                     sol_ks1.append(wavenum)
                     p_diff_sols.append(abs(left_P_solution[-1] - inside_P_solution[0]))
                     loop_ws[:] = []
                     loop_sign_check[:] = [0]
                     sol_omegas1_imag.append(omega[n])
                     sol_ks1_imag.append(wavenum)
                     p_diff_sols_imag.append(abs(left_P_solution_imag[-1] - inside_P_solution_imag[0]))
                     loop_ws_imag[:] = []
                     loop_sign_check_imag[:] = [0]
                     break
 
#              elif (abs(left_P_solution_imag[-1] - inside_P_solution_imag[0])*100/max(abs(left_P_solution_imag[-1]), abs(inside_P_solution_imag[0]))) < p_tol:
#                     sol_omegas1_imag.append((omega[k] + 1j*omega[n]).imag)
#                     sol_ks1_imag.append(wavenum)
#                     p_diff_sols_imag.append(abs(left_P_solution_imag[-1] - inside_P_solution_imag[0]))
#                     loop_ws_imag[:] = []
#                     loop_sign_check_imag[:] = [0]
...
#              elif loop_sign_check_imag[-1] < 0 and len(loop_ws_imag)>1:   #If this is negative go back through and reduce gap
#                   
#                     omega = np.linspace(loop_ws[-2], loop_ws[-1], 3) + 1j*np.linspace(loop_ws_imag[-2], loop_ws_imag[-1], 3)
#                     wavenum = wavenum
#                     itt_num =itt_num +1
      ...
      #for i in range(len(wavenumber)):
        for j in range(len(freq.real)):
          for m in range(len(freq.imag)):
            ...
            m_e = ((((wavenumber**2*vA_e**2)-((freq[j] + 1j*freq[m])-(wavenumber*mach_e))**2)*((wavenumber**2*c_e**2)-((freq[j] + 1j*freq[m])-(wavenumber*mach_e))**2))/((vA_e**2+c_e**2)*((wavenumber**2*cT_e()**2)-((freq[j] + 1j*freq[m])-(wavenumber*mach_e))**2)))

            p_e_const = rho_e*(vA_e**2+c_e**2)*((wavenumber**2*cT_e()**2)-((freq[j] + 1j*freq[m])-(wavenumber*mach_e))**2)/(((freq[j] + 1j*freq[m])-(wavenumber*mach_e))*((wavenumber**2*c_e**2)-((freq[j] + 1j*freq[m])-(wavenumber*mach_e))**2))


#######################################################################################################           
            def m0(x):    
              return ((((wavenumber**2*c_i**2)-((freq[j] + 1j*freq[m])-(wavenumber*U_i(x)))**2)*((wavenumber**2*vA_i**2)-((freq[j] + 1j*freq[m])-(wavenumber*U_i(x)))**2))/((c_i**2+vA_i**2)*((wavenumber**2*cT_i()**2)-((freq[j] + 1j*freq[m])-(wavenumber*U_i(x)))**2)))  
    
            m0_np=sym.lambdify(xx,m0(xx),"numpy")           
       ##############################################
                  
            #def D(x):    
             # return (2.*wavenumber*dU_i(x)*((((freq[j] + 1j*freq[m])-(wavenumber*U_i(x)))**2-(wavenumber**2*cT_i()**2))+((wavenumber**4*cT_i()**2*c_i**2)/((c_i**2+vA_i**2)*(((freq[j] + 1j*freq[m])-(wavenumber*U_i(x)))**2-(wavenumber**2*cT_i()**2)))))/(((freq[j] + 1j*freq[m])-wavenumber*U_i(x))*(((freq[j] + 1j*freq[m])-(wavenumber*U_i(x)))**2-(wavenumber**2*c_i**2))))           
            def D(x):    
              return (2.*wavenumber*dU_i(x)*((((freq[j] + 1j*freq[m])-wavenumber*U_i(x))**2/((((freq[j] + 1j*freq[m])-wavenumber*U_i(x))**2)-wavenumber**2*c_i**2)) - ((wavenumber**2*cT_i()**2)/((((freq[j] + 1j*freq[m])-wavenumber*U_i(x))**2)-wavenumber**2*cT_i()**2)))/((freq[j] + 1j*freq[m])-wavenumber*U_i(x))) 
       ...
       ##############################################
            ...
            def coeff(x):
                return ((wavenumber*ddU_i(x)/((freq[j] + 1j*freq[m])-wavenumber*U_i(x)))+((wavenumber*dU_i(x)*D(x))/((freq[j] + 1j*freq[m])-wavenumber*U_i(x))) - m0(x))
   
            coeff_np=sym.lambdify(xx,coeff(xx),"numpy")
       ##############################################  
   
            def P_Ti(x):    
              return (rho_i*(vA_i**2+c_i**2)*((wavenumber**2*cT_i()**2)-((freq[j] + 1j*freq[m])-(wavenumber*U_i(x)))**2)/(((freq[j] + 1j*freq[m])-(wavenumber*U_i(x)))*((wavenumber**2*c_i**2)-((freq[j] + 1j*freq[m])-(wavenumber*U_i(x)))**2))) 
            ...
            def add_P_Ti(x):
              return (-(wavenumber*dU_i(x))/((freq[j] + 1j*freq[m])-wavenumber*U_i(x)))
...
########################################################################################################
            ...
            if m_e.real < 0:
              ...
              #def dVx_dx_e_imag(Vx_e_imag, x_e_imag):
              #       return [Vx_e_imag[1], m_e.imag*Vx_e_imag[0]]
                                     
              V0 = [1e-8, 1e-15]              
              #V0 = [1e-8 + 1j*1e-8, 1e-15 + 1j*1e-15] 
              
              Ls = odeintz(dVx_dx_e, V0, lx, printmessg=0)
              #Ls_imag = odeintz(dVx_dx_e_imag, V0, lx, printmessg=0)
              
              left_solution = Ls[:,0].real*((freq[j])-wavenumber*mach_i[0])/((freq[j])-wavenumber*mach_e)
              left_solution_imag = Ls[:,0].imag*((freq[m])-wavenumber*mach_i[0])/((freq[m])-wavenumber*mach_e) 
              ...
              #def dVx_dx_i_imag(Vx_i_imag, x_i_imag):
              #      return [Vx_i_imag[1], (-D_np(x_i_imag).imag*Vx_i_imag[1] - coeff_np(x_i_imag).imag*Vx_i_imag[0])]
                  
              def objective_dvxi(dVxi): # We know that Vx(0) = 0 however do not know value of dVx at boundary inside slab so use fsolve to calculate
                    U = odeintz(dVx_dx_i, [left_bound_vx, dVxi], ix, printmessg=0)
                    #U = odeintz(dVx_dx_i, [left_bound_vx + 1j*left_bound_vx_imag, dVxi[0] + 1j*dVxi[1]], ix, printmessg=0)
                    u1 = U[:,0].real + left_bound_vx
                    #u1_imag = U[:,0].imag + left_bound_vx_imag
                    return u1[-1]#, u1_imag[-1]  


              #def objective_dvxi_imag(dVxi_imag):    # We know that Vx(0) = 0 however do not know value of dVx at boundary inside slab so use fsolve to calculate
             #       U_imag = odeintz(dVx_dx_i, [left_bound_vx_imag, dVxi_imag], ix, printmessg=0)
             #       u1_imag = U_imag[:,0].imag + left_bound_vx_imag
             #       return u1_imag[-1] 
                    
                                        
              dVxi, = fsolve(objective_dvxi, 1.)    #zero is guess for roots of equation, maybe change to find more body modes??
              #dVxi, dVxi_imag = fsolve(objective_dvxi, [1., 1.])    #zero is guess for roots of equation, maybe change to find more body modes??
              #dVxi_imag, = fsolve(objective_dvxi_imag, 1.)    #zero is guess for roots of equation, maybe change to find more body modes??
      
              # now solve with optimal dvx
              
              Is = odeintz(dVx_dx_i, [left_bound_vx, dVxi], ix)
              #Is = odeintz(dVx_dx_i, [left_bound_vx + 1j*left_bound_vx_imag, dVxi + 1j*dVxi_imag], ix)
              #Is_imag = odeintz(dVx_dx_i, [left_bound_vx_imag, dVxi_imag], ix)
              inside_solution = Is[:,0].real
              inside_solution_imag = Is[:,0].imag
              ...
              P_diff_check_imag.append((left_P_solution_imag[-1]-inside_P_solution_imag[0]))
              sign_check_imag.append(P_diff_check_imag[-1]*P_diff_check_imag[-2])
                                      
              #test_w_k_sausage.append((freq[j] + 1j*freq[m]).real/wavenumber)
              #test_p_diff_sausage.append(abs(left_P_solution[-1]) - abs(inside_P_solution[0]))
              
              all_ks.append(wavenumber)
              all_ks_imag.append(wavenumber)
              all_ws.append(freq[j])
              all_ws_imag.append(freq[m])
              ...
              if (abs(left_P_solution[-1] - inside_P_solution[0])*100/max(abs(left_P_solution[-1]), abs(inside_P_solution[0]))) < p_tol: # and (abs(left_P_solution_imag[-1] - inside_P_solution_imag[0])*100/max(abs(left_P_solution_imag[-1]), abs(inside_P_solution_imag[0]))) < p_tol:
                    ...
                    p_diff_sols.append(abs(left_P_solution[-1] - inside_P_solution[0]))
                    all_ws[:] = []
                    sol_omegas1_imag.append(freq[m])
...
#              if sign_check_imag[-1] < 0: #and (round(inside_P_solution[0]) == round(inside_P_solution[-1])): 
#                  if len(all_ws_imag)>1:
#                  
#                     if all_ks_imag[-1] != all_ks_imag[-2]:
#                       sign_check_imag[:] = []
#                       
#                      
#                     else:         
#                       omega = np.linspace(all_ws[-2], all_ws[-1], 3) + 1j*np.linspace(all_ws_imag[-2], all_ws_imag[-1], 3)
#                       wavenum = all_ks_imag[-1]
#              #        #now repeat exact same process but in foccused omega range
#                       itt_num = 0
#                       all_ws_imag[:] = []
#                       all_ws[:] = []
...
def kink(wavenumber, kink_ws, kink_ks, kink_ws_imag, kink_ks_imag, freq):
 ...
 ########################################################################################################
          ...
          if m_e.real < 0:
              ...
              loop_ws_kink_imag.append(omega[n])
              ...
              left_solution = Ls[:,0].real*((omega[k])-wavenum*mach_i[0])/((omega[k])-wavenum*mach_e)       # Vx perturbation solution for left hand side
              left_solution_imag = Ls[:,0].imag*((omega[n])-wavenum*mach_i[0])/((omega[n])-wavenum*mach_e)  # Vx perturbation solution for left hand side
             ...
             #       return u1_imag[-1] 
              ...
              P_diff_loop_check_kink_imag.append((left_P_solution_imag[-1]-inside_P_solution_imag[0]))
              loop_sign_check_kink_imag.append(P_diff_loop_check_kink_imag[-1]*P_diff_loop_check_kink_imag[-2])
              ...
              if (abs(left_P_solution[-1] - inside_P_solution[0])*100/max(abs(left_P_solution[-1]), abs(inside_P_solution[0]))) < p_tol: # and (abs(left_P_solution_imag[-1] - inside_P_solution_imag[0])*100/max(abs(left_P_solution_imag[-1]), abs(inside_P_solution_imag[0]))) < p_tol:
                     ...
                     loop_ws_kink[:] = []
                     loop_sign_check_kink[:] = [0]
                     sol_omegas_kink1_imag.append(omega[n])
                     sol_ks_kink1_imag.append(wavenum)
                     p_diff_sols_kink_imag.append(abs(left_P_solution_imag[-1] - inside_P_solution_imag[0]))
                     loop_ws_kink_imag[:] = []
                     loop_sign_check_kink_imag[:] = [0]
                     break
 
#              elif (abs(left_P_solution_imag[-1] - inside_P_solution_imag[0])*100/max(abs(left_P_solution_imag[-1]), abs(inside_P_solution_imag[0]))) < p_tol:
#                     sol_omegas_kink1_imag.append((omega[k] + 1j*omega[n]).imag)
#                     sol_ks_kink1_imag.append(wavenum)
#                     p_diff_sols_kink_imag.append(abs(left_P_solution_imag[-1] - inside_P_solution_imag[0]))
#                     loop_ws_kink_imag[:] = []
#                     loop_sign_check_kink_imag[:] = [0]
...
#              elif loop_sign_check_kink_imag[-1] < 0 and len(loop_ws_kink_imag)>1:   #If this is negative go back through and reduce gap
#   
#                     omega = np.linspace(loop_ws_kink[-2], loop_ws_kink[-1], 3) + 1j*np.linspace(loop_ws_kink_imag[-2], loop_ws_kink_imag[-1], 3)
#                     wavenum = wavenum
#                     #now repeat exact same process but in foccused omega range
...
#######################################################################################################           
       ...
       ##############################################
            ...
            #def D(x):    
             # return (2.*wavenumber*dU_i(x)*((((freq[j] + 1j*freq[m])-(wavenumber*U_i(x)))**2-(wavenumber**2*cT_i()**2))+((wavenumber**4*cT_i()**2*c_i**4)/((c_i**2+vA_i**2)*(((freq[j] + 1j*freq[m])-(wavenumber*U_i(x)))**2-(wavenumber**2*cT_i()**2)))))/(((freq[j] + 1j*freq[m])-wavenumber*U_i(x))*(((freq[j] + 1j*freq[m])-(wavenumber*U_i(x)))**2-(wavenumber**2*c_i**2))))    
...
########################################################################################################
            ...
            if m_e.real < 0:
              ...
              Ls = odeintz(dVx_dx_e, V0, lx, printmessg=0)
              #Ls_imag = odeintz(dVx_dx_e, V0, lx, printmessg=0)
              
              left_solution = Ls[:,0]*((freq[j])-wavenumber*mach_i[0])/((freq[j])-wavenumber*mach_e)  # Vx perturbation solution for left hand side
              #left_solution_imag = Ls[:,0].imag*((freq[m])-wavenumber*mach_i[0])/((freq[m])-wavenumber*mach_e) # Vx perturbation solution for left hand side
              
              left_P_solution = p_e_const.real*Ls[:,1]  # Pressure perturbation solution for left hand side
              #left_P_solution_imag = p_e_const.imag*Ls[:,1].imag  # Pressure perturbation solution for left hand side     
             ...
             #       return u1_imag[-1] 
              ...
              inside_P_solution = p_i_const.real*(Is[:,1] - add_p_i_const.real*Is[:,0])
              #inside_P_solution_imag = p_i_const.imag*(Is[:,1].imag - add_p_i_const.imag*Is[:,0].imag)
              ...
              #P_diff_check_kink_imag.append((left_P_solution_imag[-1]-inside_P_solution_imag[0]))
              #sign_check_kink_imag.append(P_diff_check_kink_imag[-1]*P_diff_check_kink_imag[-2])
       
              test_p_diff_kink.append(abs(left_P_solution[-1]) - abs(inside_P_solution[0]))
  
              all_ks_kink.append(wavenumber)
              all_ks_kink_imag.append(wavenumber)
              all_ws_kink.append(freq[j])
              all_ws_kink_imag.append(freq[m])
              ...
              if (abs(left_P_solution[-1] - inside_P_solution[0])*100/max(abs(left_P_solution[-1]), abs(inside_P_solution[0]))) < p_tol:  # and (abs(left_P_solution_imag[-1] - inside_P_solution_imag[0])*100/max(abs(left_P_solution_imag[-1]), abs(inside_P_solution_imag[0]))) < p_tol:
                    ...
                    p_diff_sols_kink.append(abs(left_P_solution[-1] - inside_P_solution[0]))
                    all_ws_kink[:] = []
                    sol_omegas_kink1_imag.append(freq[m])
                    sol_ks_kink1_imag.append(wavenumber)
                    p_diff_sols_kink_imag.append(abs(left_P_solution_imag[-1] - inside_P_solution_imag[0]))
                    all_ws_kink_imag[:] = []
                    
#              if (abs(left_P_solution_imag[-1] - inside_P_solution_imag[0])*100/max(abs(left_P_solution_imag[-1]), abs(inside_P_solution_imag[0]))) < p_tol:
#                    sol_omegas_kink1_imag.append((freq[j] + 1j*freq[m]).imag)
#                    sol_ks_kink1_imag.append(wavenumber)
#                    p_diff_sols_kink_imag.append(abs(left_P_solution_imag[-1] - inside_P_solution_imag[0]))
#                    all_ws_kink_imag[:] = []
...
#              if sign_check_kink_imag[-1] < 0: #and (round(inside_P_solution[0]) == round(inside_P_solution[-1])): 
#                  if len(all_ws_kink_imag)>1:
#                  
#                     if all_ks_kink_imag[-1] != all_ks_kink_imag[-2]:
#                       sign_check_kink_imag[:] = []
#                                          
#                     else:         
#                       omega = np.linspace(all_ws_kink[-2], all_ws_kink[-1], 3) + 1j*np.linspace(all_ws_kink_imag[-2], all_ws_kink_imag[-1], 3)
#                       wavenum = all_ks_kink_imag[-1]
#              #        #now repeat exact same process but in foccused omega range
#                       itt_num = 0
#                       all_ws_kink[:] = []
#                       all_ws_kink_imag[:] = []
...
wavenumber = np.linspace(0.01,2.5,20)      #(1.5, 1.8), 5    # for test split k range by 2
...
if __name__ == '__main__':
    ...
    for k in wavenumber:
      for i in range(len(speeds)-1):
         ...
         #test_freq = np.linspace(speeds[i]*k, speeds[i+1]*k, 40.) + 1j*np.linspace(speeds[i]*k, speeds[i+1]*k, 40.)  
         test_freq = np.linspace(speeds[i]*k, speeds[i+1]*k, 10.) + 1j*np.linspace(-0.25, 0.25, 10.)
...
with open('flow_complex_coronal_test.pickle', 'wb') as f:
...
plt.savefig("uniform_superflow_complex_coronal_test.png")
